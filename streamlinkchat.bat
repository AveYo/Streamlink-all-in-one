goto="Streamlinkchat.Init" /* 
::     url input + stream choice dialogs and hidecmd launcher v7 final
:: Save as Streamlinkchat.bat in Streamlink folder, can be called using [Win+R] Run-menu after first launch
...
::----------------------------------------------------------------------------------------------------------------------------------
:"Streamlinkchat.Init" Hybrid initialization with HideCmd and arguments pass-trough after self-restart 
@echo off & setlocal & pushd "%~dp0" & if "%1"=="--self--restart--" call :"Streamlinkchat.Batch" %--init--% & endlocal & exit/b  
...
// end of hybrid batch script by AveYo